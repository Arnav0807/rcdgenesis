    ...
    <>
      <section className="relative overflow-hidden bg-ink text-[#ece6d8] py-16 md:py-[70px]">
        ...
        <div className="relative mx-auto max-w-[1180px] px-7">
          ...
          <motion.p initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.16}} className="max-w-[58ch] text-[1.06rem] text-[#aab9c9]">Every active member of the club, in one place. Home addresses and personal contact details are kept off this public page.</motion.p>
        </div>
      </section>
              ...
              <Reveal key={m.id} delay={Math.min(i*0.04,0.4)} y={16}>
                <div className="bg-bone-2 rounded-sm p-6 flex flex-col gap-4">
                  <div className="flex items-center gap-4">
                    {m.photo
                      ? <img src={m.photo} alt="" className="h-12 w-12 rounded-full object-cover shrink-0"/>
                      : <div className="h-12 w-12 rounded-full bg-ink text-gold-soft font-display font-semibold flex items-center justify-center text-[1.05rem] shrink-0">{initials(m.firstName,m.lastName)}</div>
                    }
                    <div>
                      <div className="font-display text-[1.12rem] leading-tight">{m.firstName} {m.lastName}</div>
                      <div className="font-mono text-[0.74rem] tracking-[0.04em] text-teal">{m.city}</div>
                    </div>
                  </div>
                  <div className="flex items-center justify-between gap-3 pt-3 border-t border-[#1c2430]/10">
                    <span className="font-mono text-[0.68rem] tracking-[0.08em] uppercase text-[#5a5347]">Member since {m.since}</span>
                    {m.role && <span className="font-mono text-[0.6rem] tracking-[0.08em] uppercase px-2 py-1 rounded-full bg-gold/20 text-[#8a5a16] text-right">{m.role}</span>}
                  </div>
                </div>
              </Reveal>
            ))}
