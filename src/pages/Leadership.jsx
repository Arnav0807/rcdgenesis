...
function Field({ k, v }) {
  return (
    <>
      <span className="font-mono text-[0.68rem] tracking-[0.08em] uppercase text-[#aab9c9] pt-[3px] whitespace-nowrap">{k}</span>
...
function Card({ entry }) {
  return (
    <div className="bg-ink text-[#ece6d8] rounded-sm px-7 py-6 max-w-[460px] w-full">
      <div className="font-mono text-[0.74rem] tracking-[0.1em] uppercase text-[#aab9c9] flex items-center gap-2.5 mb-2.5">
        {entry.year}
        {entry.tag && <span className={entry.tag==='Charter Year'?'font-mono text-[0.62rem] uppercase px-2 py-0.5 rounded-full bg-gold text-ink':'font-mono text-[0.62rem] uppercase px-2 py-0.5 rounded-full border border-gold-soft text-gold-soft'}>{entry.tag}</span>}
      </div>
      <h3 className="font-display text-[1.5rem] mb-3.5">{entry.president}</h3>
      <div className="grid grid-cols-[auto_1fr] gap-x-3.5 gap-y-1.5 text-[0.96rem]">
      ...
      {entry.note && <div className="mt-4 pt-3.5 border-t border-white/15 text-[0.9rem] text-[#aab9c9] italic">{entry.note}</div>}
    </div>
  )
}
...
function Node({ entry }) {
  if (entry.tag==='Charter Year') return (
    <div className="relative h-[34px] w-[34px] rounded-full z-10" style={{background:'radial-gradient(circle, #e2a33b 0%, #e2a33b 55%, transparent 56%)'}}>
    ...
    <>
      <section className="relative overflow-hidden bg-ink text-[#ece6d8] py-16 md:py-[70px]">
        ...
        <div className="relative mx-auto max-w-[1180px] px-7">
          ...
          <motion.p initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.16}} className="max-w-[52ch] text-[1.06rem] text-[#aab9c9]">From the charter signing in September 2023 to the present term, every Rotary year at Delhi Genesis pairs a president, secretary, and AG with the District Governor serving District 3011 that same year.</motion.p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1180px] px-7">
          <div className="relative">
            ...
            {leadership.map((entry,i) => {
              const isEven = i%2===0
              return (
                <div key={entry.id??entry.year} className="relative">
                  <div className="hidden md:grid md:grid-cols-[1fr_60px_1fr] items-start mb-14 last:mb-0">
                    <div className={isEven?'flex justify-end pr-10':''}>
                      {isEven && <Reveal y={20} delay={0.05*i}><Card entry={entry}/></Reveal>}
                    </div>
