    ...
    <>
      <section className="relative overflow-hidden bg-ink text-[#ece6d8] py-16 md:py-[70px]">
        ...
        <div className="relative mx-auto max-w-[1280px] px-5">
          ...
          <motion.h1 initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.08}} className="font-display font-semibold leading-[1.08] my-5 text-[2.2rem] md:text-[3.2rem]">The Board of Directors.</motion.h1>
          <motion.p initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{duration:0.6,delay:0.16}} className="max-w-[54ch] text-[1.04rem] text-[#aab9c9]">Current term led by {club.currentPresident}.</motion.p>
        </div>
      </section>
      <section className="py-16 md:py-[90px]">
        <div className="mx-auto max-w-[1280px] px-5 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {board.map((b,i)=>(
            <Reveal key={b.id} delay={Math.min(i*0.05,0.4)} y={16}>
              <div className={`rounded-sm p-7 flex flex-col gap-3 h-full ${b.name?'bg-ink text-[#ece6d8]':'border-[1.5px] border-dashed border-[#1c2430]/25 text-[#5a5347]'}`}>
