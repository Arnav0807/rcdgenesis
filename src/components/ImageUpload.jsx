...
export default function ImageUpload({ value, onChange, label='Photo', circular=false }) {
  ...
  function handleFile(e) {
    ...
    if (!file) return
    if (file.size > 3_500_000) { alert('Please use an image under ~3 MB.'); return }
    ...
    <div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-2">
          <button type="button" onClick={() => ref.current?.click()} className="font-mono text-[0.7rem] tracking-[0.06em] uppercase px-3 py-2 rounded-sm border border-[#1c2430]/25 hover:bg-bone-2 transition-colors">
