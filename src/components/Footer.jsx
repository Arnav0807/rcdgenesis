    ...
    <footer className="bg-ink text-[#aab9c9] text-[0.9rem] py-10">
      <div className="mx-auto max-w-[1280px] px-5 flex flex-wrap items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          />
          <div>
            <div className="font-display text-[#ece6d8] text-[1rem] mb-0.5">
              {club.name}
            </div>
            <div className="text-[0.8rem]">
              Chartered {club.chartered} · District {club.district} · RI Club ID{" "}
              {club.clubId}
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-5">
          {[
            ...
            ["/board", "Board"],
            ["/members", "Members"],
            ["/activities", "Fellowship & Projects"],
          ].map(([to, l]) => (
            <Link
              key={to}
              to={to}
              className="hover:text-[#ece6d8] transition-colors"
            >
              {l}
            </Link>
          ))}
