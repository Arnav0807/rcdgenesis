...
export default function MemberSelect({ value, onChange, members, placeholder='— Not yet on record —' }) {
  ...
  if (mode==='custom') return (
    <div className="space-y-1.5">
      <input className={iCls} value={value||''} onChange={e=>onChange(e.target.value)} placeholder="Type a name"/>
      <button type="button" onClick={()=>{setMode('select');onChange('')}} className="font-mono text-[0.66rem] tracking-[0.06em] uppercase text-teal hover:underline">Choose from members instead</button>
    </div>
  )
  return (
    <select className={iCls} value={value||''} onChange={e=>{ if(e.target.value==='__other__'){setMode('custom');onChange('')} else onChange(e.target.value) }}>
